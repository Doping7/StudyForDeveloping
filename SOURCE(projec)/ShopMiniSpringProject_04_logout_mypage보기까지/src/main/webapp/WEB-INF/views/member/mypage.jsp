...
전화번호:<select name="phone1">
  <option value="010" selected='<c:if test="${login.phone1 ==010}">selected</c:if>'
  >010</option><!-- html태그 사이에서 jstl태그 사용  -->
  <option value="011" selected='<c:if test="${login.phone1 ==011}">selected</c:if>'
...
이메일:<input type="text" value="${login.email1}" name="email1" id="email1">@
       ...
       <select  id="emailSelect">
        <option value="daum.net" selected='<c:if test="${login.email2 =='daum.net'}">selected</c:if>'>daum.net</option>
        <option value="naver.com"selected='<c:if test="${login.email2 =='naver.net'}">selected</c:if>'>>naver.com</option>
       </select>
<br>
